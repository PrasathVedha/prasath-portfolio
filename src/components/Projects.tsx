...
const AnimatedCloud = () => <svg viewBox="0 0 140 80" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full object-contain">
    <g>
      <motion.ellipse cx="70" cy="45" rx="35" ry="18" fill="#fff" initial={{
      x: -30
    }} animate={{
      x: [-30, 30, -30]
    }} transition={{
      ...
      filter: 'drop-shadow(0px 6px 8px rgba(120,120,140,0.08))'
    }} />
...
const ProjectCard = ({
  ...
  project: any;
  index: number;
}) => <motion.div className="data-card overflow-hidden card-hover" initial={{
  opacity: 0,
  y: 20
}} whileInView={{
  opacity: 1,
  ...
  delay: index * 0.1
    ...
    <div className="h-48 bg-gradient-to-r from-data-blue to-data-purple flex items-center justify-center relative overflow-hidden">
      {typeof project.image === 'string' ? <img src={project.image} alt={project.title} className="w-full h-full object-cover" onError={e => {
    ...
    <div className="p-6">
      ...
      <div className="mb-4">
        ...
        <ul className="list-disc pl-5 space-y-1 text-gray-600">
          {project.points.slice(0, 3).map((point: string, i: number) => <li key={i} className="text-sm">{point}</li>)}
        </ul>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap">
          {project.technologies.map((tech: string, i: number) => <span key={i} className="skill-badge">
              {tech}
            </span>)}
        </div>
      </div>
...
const Projects = () => {
  return <section id="projects" className="py-20 bg-data-lightGray">
